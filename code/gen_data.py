...
#       assert (len(spacy_sent) == len(sents[sent_no])) # make sure the length of sentence parsed by spacy is the same as original sentence.
        ...
        # 输入形式  e = [(1, 2), (2, 3), (3, 4)]  根据这种大小来产生对应的图结构
...
def Init(data_file_name, rel2id, max_length = 512, is_training = True, suffix=''):
    ...
    for i in range(len(ori_data)):
        ...
        Ls = [0]
        L = 0
        # 计算该sentence前所有sentence的长度 采用真前缀的计数方法
        ...
        new_labels = []
        for label in labels:
            # 这个循环的内容 对于r进行映射 构建mention之间的关联 标定测试集和验证集当中相似的数据
            rel = label['r']
            ...
            '''这段代码寻找句子之间mention的各种联系'''
            if suffix=='_train':
                # 交互的信息是在多个mention之间  三元组可能在内部之间
